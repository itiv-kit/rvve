...
void tanh_activation(int64_t *data, int32_t length)
{
  ...
  { 
    ...
    temp_val = (int32_t)data[i];
    asm volatile(
            ".insn r 0b0001011, 0b001, 0b0000000,  %0, %1,%1\n\t"
            "sw   %0, 0(%2)\n\t"
    :
    :"r"((temp_val)), "r"((temp_val)), "r"(&temp_val)
    :"memory" );
    data[i]=(int64_t )temp_val;
  }
...
void tanh_activation(int64_t *data, int32_t length)
{
  for(uint32_t i = 0; i<length; i++)
  {
    int32_t index_val = 0;
    index_val = data[i] << 3;
    index_val = index_val + 0b00000000000000000011111100000000; //+31,5  Q32.9
    index_val = index_val +  (1 << 8);
...
void logistic(int64_t *data, int32_t length)
{
  ...
  {  
    int32_t index_val = 0;
    index_val = data[i] << 2;
    index_val = index_val + 0b00000000000000000011111100000000; //+127.5 Q32.9
    index_val = index_val + (1<<8);
    index_val = index_val >> 9;
    if(index_val >63){
      data[i]=127;
