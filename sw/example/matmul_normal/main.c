...
int main(void)
{
  ...
  uint32_t a_height = 50;
  uint32_t b_width = 50;
  uint32_t a_width_b_height = 100;
...
// Matrix Multiplication mat_a*mat_b = mat_res
stride = b_width ;
mat_b_help = mat_b;
mat_a_help = mat_a;
...
for (uint32_t k = 0; k < a_height; k++)  {
  ...
  for (uint32_t i = 0; i < b_width; i++){
    ...
    acc = 0;
      ...
      {
         acc+= mat_a[k*a_width_b_height+m] * mat_b[m*b_width+i];
