...
int main()
{
   ...
   total_scaling_hidden_update.scaling_factor.value = 0b00000000100001001010010101010110;// (cell_state_scaling factor * tanh.scaling_Factor)/hidden_state_scaling factor
 ...
 for(int32_t i = 0; i < out_length; i++)
   ...
   for(int32_t k=0; k<time_steps; k++)
   {
      ...
      mat_vec_mul( &input_weights[0][0] , input_data,  input_result,  out_length,in_length );
      mat_vec_mul(&forget_weights[0][0] ,input_data,    input_result+out_length,  out_length,in_length );
      mat_vec_mul( &cell_weights[0][0] , input_data,  input_result+out_length*2,  out_length,in_length );
      mat_vec_mul( &output_weights[0][0] ,input_data,   input_result+out_length*3,  out_length,in_length );


      mat_vec_mul(&rec_input_weights[0][0] , &state->hidden[0]  ,   recurrent_result, out_length ,out_length );
      mat_vec_mul( &rec_forget_weights[0][0]  ,&state->hidden[0],   recurrent_result+out_length,out_length  ,out_length );
      mat_vec_mul(&rec_cell_weights[0][0]  , &state->hidden[0],   recurrent_result+out_length*2, out_length,out_length );
      mat_vec_mul(&rec_output_weights[0][0]  ,&state->hidden[0],    recurrent_result+out_length*3, out_length,out_length );
