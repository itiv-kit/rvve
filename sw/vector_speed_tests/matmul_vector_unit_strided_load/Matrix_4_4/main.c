...
int main(void)
{
  ...
  for (uint32_t k = 0; k < b_width; k++)  {
    ...
    for (uint32_t i = 0; i < a_height; i++){
      ...
      asm volatile(
      ".insn i 0b1111011, 0b000,x30, %0,  0b000000000100\n\t"
      ".insn i 0b1111011, 0b000,x31, %1,  0b000000000100\n\t"
      :
      :"r"(mat_a_help), "r"(mat_b_help)
      :  "x30", "x31" );

      asm volatile(
      ".insn r 0b1011011, 0b011, 0x00, x30, x31, x30\n\t"         
      :
      :"r"(mat_a_help), "r"(mat_b_help)
      : "x30", "x31"  );
