...
int main(void)
{
  ...
  uint32_t a_height = 20;
  uint32_t b_width = 20;
  uint32_t a_width_b_height = 20;
...
for (uint32_t k = 0; k < b_width; k++)  {
  ...
  for (uint32_t i = 0; i < a_height; i++){
    ...
    asm volatile(
    ".insn i 0b1111011, 0b000,x22, %0,  0b000000000100\n\t"
    ".insn i 0b1111011, 0b000,x27, %1,  0b000000000100\n\t"
    :
    :"r"(mat_a_help), "r"(mat_b_help)
    :  "x22","x23", "x24", "x25", "x26", "x27", "x28","x29","x30", "x31" );

    asm volatile(
    ".insn r 0b1011011, 0b011, 0x00, x22, x27, x22\n\t"         
    :
    :"r"(mat_a_help), "r"(mat_b_help)
    :  "x22","x23", "x24", "x25", "x26", "x27", "x28","x29","x30", "x31"  );
